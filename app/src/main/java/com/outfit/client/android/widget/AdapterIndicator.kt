...
class AdapterIndicator @JvmOverloads constructor(
	context: Context,
	...
	private val internalRecyclerScrollListener: InternalRecyclerScrollListener =
	...
	private var onPageChangeCallback: ViewPager2.OnPageChangeCallback =
	...
	init {
		attrs?.let {
			val typedArray = context.theme.obtainStyledAttributes(
				attrs,
				R.styleable.AdapterIndicator,
				0,
				0
			)
			dotCount = typedArray.getInteger(
				R.styleable.AdapterIndicator_dotCount,
				DEFAULT_DOT_COUNT
			)
			fadingDotCount = typedArray.getInt(
				R.styleable.AdapterIndicator_fadingDotCount,
				DEFAULT_FADING_DOT_COUNT
			)
			dotRadiusPx = typedArray.getDimensionPixelSize(
				R.styleable.AdapterIndicator_dotRadius,
				dotRadiusPx
			)
			selectedDotRadiusPx = typedArray.getDimensionPixelSize(
				R.styleable.AdapterIndicator_selectedDotRadius,
				selectedDotRadiusPx
			)

			val dotColor = when (typedArray.hasValue(R.styleable.AdapterIndicator_dotColor)) {
				true -> typedArray.getColor(R.styleable.AdapterIndicator_dotColor, 0)
				false -> {
			...
			val selectedDotColor =
				when (typedArray.hasValue(R.styleable.AdapterIndicator_selectedDotColor)) {
					true -> typedArray.getColor(R.styleable.AdapterIndicator_selectedDotColor, 0)
					false -> {
						val typedValue = TypedValue()
			...
			dotSeparationDistancePx = typedArray.getDimensionPixelSize(
				R.styleable.AdapterIndicator_dotSeparation,
				dotSeparationDistancePx
			)
			supportRtl =
				typedArray.getBoolean(R.styleable.AdapterIndicator_supportRTL, false)
			verticalSupport = typedArray.getBoolean(
				R.styleable.AdapterIndicator_verticalSupport,
				false
			)
	...
	/**
	 * Set the dimensions of the AdapterIndicator.
	...
	fun attachTo(recyclerView: RecyclerView?) {
		...
		viewPager = null
		currentRecyclerView?.removeOnScrollListener(internalRecyclerScrollListener)

		currentRecyclerView = recyclerView
		currentRecyclerView?.addOnScrollListener(internalRecyclerScrollListener)
	}
	...
	private fun getPagerItemCount(): Int = when {
		currentRecyclerView != null -> currentRecyclerView?.adapter?.itemCount ?: 0
	...
	internal inner class InternalRecyclerScrollListener : RecyclerView.OnScrollListener() {
		...
		private fun getMostVisibleChild(): View? {
			...
			for (i in currentRecyclerView?.layoutManager?.childCount!! - 1 downTo 0) {
				val child = currentRecyclerView?.layoutManager?.getChildAt(i)
				if (child != null) {
		...
		private fun setIntermediateSelectedItemPosition(mostVisibleChild: View) {
			with(currentRecyclerView?.findContainingViewHolder(mostVisibleChild)?.adapterPosition!!) {
